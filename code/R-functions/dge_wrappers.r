...
##' the likelihood ratio tests or "QLF" for the  quasi-likelihood F-tests. Also "qCML" for exact test, althought it
##' not a GLM approach
##'
  ...
  )
{
  ...
  # Processing SummarizedExperiment object
  if(class(de_obj) == 'SummarizedExperiment'){
    de_obj <- DGEList(
      counts = assay(de_obj, assay_to_DGEList),
      group = colData(de_obj)[[group_var]],
      samples = colData(de_obj) %>% data.frame(check.names = FALSE),
      genes = rowData(de_obj) %>% data.frame(check.names = FALSE)
    )
  }
  ...
  # qCML - exact test approach
  if(glm_approach == 'qCML') {
    dge_lrt <- exactTest(de_obj)
  }
