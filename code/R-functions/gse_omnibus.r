...
gse_omnibus <- function(
  ...
  args_goseq = list(genome = "hg19", id = "knownGene", method = "Wallenius"),
  args_gprofiler = list(
  ...
  args_gse = list(
    # BPPARAM = BiocParallel::MulticoreParam(1), # better to indicate this paramter, otherwise fgsea will take all the available CPU from the computer
    minGSSize = 10,
    maxGSSize = 500,
    pvalueCutoff = 1),
...
)
{
  ...
  if((run_enricher | run_all_ora) & !is.null(gmt_files) ) {
    ...
    gse_res$enricher <- map(feature_lists_de_fdr_fc, function(x) {
      res <- foreach(use_gsets = gmt_gsets) %do% {
        ...
        if(!is.null(enr.i)) {
          enr.i <- enrichplot::pairwise_termsim(enr.i)
    ...
    gse_res$enrichGO <- map(feature_lists_de_fdr_fc, function(x) {
      res <- foreach(ontology = go_ontologies) %do% {
        ...
        if(!is.null(enr.i)) {
          ...
          if(go_simplify) {
            enr.i.tosim <- enr.i
            message("Running simplify with P threshold : ", go_simplify_p)
            enr.i.tosim@result <- enr.i.tosim@result %>% filter(pvalue <= go_simplify_p)
            enr.i.sim <- clusterProfiler::simplify(enr.i.tosim, cutoff=go_simplify_cutoff, by="p.adjust", select_fun=min)
            rm(enr.i.tosim)
  ...
  # Clean memory
  rm(feature_fc_list, eg_stats, eg_stats_de_fdr_fc, eg_fc_list, feature_lists_de_fdr_fc, eg_lists_de_fdr_fc)
...
gse_edger_omnibus <- function(
  ...
  # Clean memory
  rm(object, gmt_gsets_indices, eg)
