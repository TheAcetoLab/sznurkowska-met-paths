gse.dotplot <- function(object, x = "p.adjust", y = "Description", color = "p.adjust", size = "setSize", additional_vars = NULL,
  ...
  if(!is.null(additional_vars)){
    df <- cbind(df, object %>% dplyr::select(all_of(additional_vars)))
  }

  df <- df %>% head(showCategory)
...
##' from enrichplot : https://rdrr.io/bioc/enrichplot/src/R/utilities.R
...
##'  @importFrom enrichplot : https://rdrr.io/bioc/enrichplot/src/R/utilities.R
